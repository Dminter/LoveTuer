...
public class RetryHandler implements HttpRequestRetryHandler {
    ...
    public boolean retryRequest(IOException exception, int executionCount, HttpContext context) {
        if (L.D) {
            L.w("cause exception name = " + exception.getClass().getSimpleName());
            L.w("connect retry number " + executionCount);
        }
        boolean retry = true;
        ...
        if (retry) {
            ...
            // 同一个错误只会重试两次
            CheckedExceptionHandler.handleException(exception);
        }
